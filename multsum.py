...
def normalize(m):
  max_v = 0.0
  mr = numpy.zeros((m.shape[0],m.shape[1]))
  #Get the  max_v:
...
def summarize_matrix_files_u(matrix, clustering_matrix, documents, stopwordsFilename=DEFAULT_STOPWORDS,
                           length=DEFAULT_SUMMARY_LENGTH, unit=UNIT_WORDS, output_numbers=True,
                           use_aggregate_for_clustering=True, quiet=False):
  matrices = matrix
